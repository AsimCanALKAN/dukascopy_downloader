...
def normalize_tick(symbol, day, time, ask, bid, ask_vol, bid_vol):
    ...
    if any(map(lambda x: x in symbol.lower(), ['usdrub', 'xagusd', 'xauusd', 'jpy', "deuidxeur"])):
        point = 1000
    else:
        point = 1000
...
def main():
    parser = optparse.OptionParser(usage='Usage: python {} [options] <symbol> <download_type> <year>'.format(os.path.basename(__file__)))
#    parser = optparse.OptionParser(usage='Usage: python {} [options] <symbol> <start: yyyy-mm-dd> <end: yyyy-mm-dd>'.format(os.path.basename(__file__)))
    ...
    symbol = args[0]
    year = args[2]
    download_type = args[1]
    rangeInt = 0 # Burasi onemli fora girmez
    if download_type == 'monthly':
        rangeInt = 14
    elif download_type == 'yearly':
        rangeInt = 1
    for i in range(1,rangeInt):
        if download_type == 'monthly':

            startDay = 1
            try:
                endDay =calendar.monthrange(int(args[2]), i)[1]
            except:
                endDay = 31
        elif download_type == 'yearly':
            startDay = 1
        ...
        start_date = datetime.strptime(str(year)+'-'+monthStr+'-'+str(startDay), '%Y-%m-%d')
        end_date = datetime.strptime(str(year)+'-'+monthStr+'-'+str(endDay), '%Y-%m-%d')
        print(f"Downloading Start Time: {start_date} -- End Time: {end_date}")
        output_dir = options.d
        output_suffix = f'_{options.c}' if options.c is not None else ''
        output_csv = f'{output_dir}/{symbol.replace("/", "_")}_{start_date.strftime("%Y-%m-%d")}_{end_date.strftime("%Y-%m-%d")}{output_suffix}.csv'
